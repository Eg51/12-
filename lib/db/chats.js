...
export async function addMessage(roomId, messageData) {
  try {
    ...
    const newMessage = {
      ...
      timestamp: new Date(),
      read: false,
      expireAt: null // ⏳ Unread messages live forever
    };
...
/**
 * Get messages for a room (and auto-cleanup expired read messages)
 * @param {string} roomId - Room ID
 * @param {number} limit - Maximum number of messages
 * @returns {Promise<Array>}
 */
export async function getRoomMessages(roomId, limit = 50) {
  try {
    ...
    // ⏳ STEP 1: Auto-delete messages that have been read for > 60 seconds
    await chatsCollection.updateOne(
      { _id: roomObjectId },
      {
        $pull: {
          messages: {
            read: true,
            expireAt: { $lt: now } // Check if 1 minute has passed
          }
        }
      }
    );

    // STEP 2: Fetch the remaining messages
...
/**
 * Mark messages as read for a user (sets the 1-minute countdown)
 * @param {string} roomId - Room ID
 * @param {string} userId - User ID
 * @returns {Promise<boolean>}
 */
export async function markMessagesAsRead(roomId, userId) {
  try {
    const chatsCollection = await getChatsCollection();
    const expireTime = new Date(Date.now() + 60000); // ⏳ Expires in exactly 1 minute
    ...
    const result = await chatsCollection.updateOne(
      ...
      { 
        ...
        $set: { 
          'messages.$[elem].read': true,
          'messages.$[elem].expireAt': expireTime, // Schedules for auto-deletion
          updatedAt: new Date()
        }
      },
      {
